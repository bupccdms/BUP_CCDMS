...
namespace qms.DAL
{
    public class DALDailyBreak
    {
        ...
        public DataTable GetAll(int? department_id, string user_id)
        {
            try
            {
                manager.AddParameter(new OracleParameter() { ParameterName = "department_id", Value = department_id });
        ...
        public int GetCountByUserId(string user_id)
        {
            try
            {
                ...
                OracleParameter param = new OracleParameter("PO_IS_BREAK_ASSIGNED", OracleDbType.Int32);
                param.Direction = ParameterDirection.Output;
                manager.AddParameter(param);

                manager.CallStoredProcedure_Select("USP_DAILYBREAK_COUNTBYUSERID");

                if (param.Value != DBNull.Value)
                    return (int)((Oracle.DataAccess.Types.OracleDecimal)param.Value).Value;
                else
                    return 0;
        ...
        private void MapParameters(tblDailyBreak dailyBreak)
        {
            ...
            manager.AddParameter(new OracleParameter("p_device_id", dailyBreak.device_id));
