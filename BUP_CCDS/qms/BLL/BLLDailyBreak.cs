...
namespace qms.BLL
{
    ...
    {
        public List<VMDailyBreak> GetAll(int? department_id, string user_id)
        {
            DALDailyBreak dal = new DALDailyBreak();
            DataTable dt = dal.GetAll(department_id, user_id);
        ...
        {
            DALDailyBreak dal = new DALDailyBreak();
            return dal.GetCountByUserId(user_id);
        }
        ...
        internal tblDailyBreak ObjectMapping(DataRow row)
        {
            ...
            dailyBreak.device_id = Convert.ToInt32(row["device_id"] == DBNull.Value ? 0 : row["device_id"]);
            dailyBreak.break_type_id = Convert.ToInt32(row["device_no"] == DBNull.Value ? null : row["device_no"].ToString());
            dailyBreak.start_time = Convert.ToDateTime(row["location"] == DBNull.Value ? null : row["location"].ToString());
            dailyBreak.user_id =(row["department_id"] == DBNull.Value ? null : row["department_id"].ToString());
            dailyBreak.end_time = Convert.ToDateTime(row["department_id"] == DBNull.Value ? null : row["department_id"].ToString());
            dailyBreak.remarks = (row["department_id"] == DBNull.Value ? null : row["department_id"].ToString());
        ...
        internal List<VMDailyBreak> ObjectMappingListVM(DataTable dt)
        {
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                dailyBreak.department_name = (row["department_name"] == DBNull.Value ? null : row["department_name"].ToString());
                dailyBreak.device_no = (row["device_no"] == DBNull.Value ? null : row["device_no"].ToString());
